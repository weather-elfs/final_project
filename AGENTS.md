...
> **현재 브랜치 전환 기준:** 이 문서가 포함된 초기 설계·공통 설정 묶음을 원격
> `develop`에 최초 게시할 때까지만 `develop`에서 작업한다. 게시 이후 데이터 출처
> 조사·스키마·대시보드 설계·기능 코드 등 실제 프로젝트 내용은 모두 최신 `develop`에서
> `feature/*`를 분기해 작업한다. 원격 `develop` 최초 생성 1회를 제외한 직접 push는
> 금지하며, 부트스트랩의 승인·보호 절차는 `docs/conventions/git.md`를 따른다.
