...
```bash
pytest              # 전체 실행
pytest tests/test_preprocess.py   # 특정 파일
pytest -k visibility              # 이름 필터
```

## 커밋/푸시 시 pytest 강제 여부 (결정 대기)

두 가지 선택지를 두고 팀이 결정한다.
...
- 결정 전까지 `.pre-commit-config.yaml`의 pytest 훅은 **주석 처리**해 둔다.
- 결정되면 이 표의 선택을 반영하고, 해당 훅의 주석을 해제한다.
